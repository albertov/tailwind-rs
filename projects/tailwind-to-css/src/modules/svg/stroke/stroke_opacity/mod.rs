...
#[derive(Debug, Clone)]
pub struct TailwindStrokeOpacity {
    percent: NumericValue,
}

impl Display for TailwindStrokeOpacity {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "stroke-opacity-{}", self.percent)
    }
}

impl TailwindInstance for TailwindStrokeOpacity {
...
impl TailwindStrokeOpacity {
    /// https://tailwindcss.com/docs/stroke-opacity (conceptual)
    pub fn parse(pattern: &[&str], arbitrary: &TailwindArbitrary) -> Result<Self> {
        ...
        let percent = match pattern {
            ...
            _ => return Err(TailwindError::syntax_error("Unknown stroke-opacity value")),
        };
        Ok(Self { percent })
    }
}